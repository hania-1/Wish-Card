...
const DynamicConfetti: any = dynamic(() => import('react-confetti'), { ssr: false });
...
export default function BirthdayWish() {
  ...
  const [birthdayWish, setBirthdayWish] = useState<string>("") // Store birthday wish
  const [wished, setWished] = useState<boolean>(false) // Check if already wished
  ...
  // Function to handle birthday wish submission
  const handleWish = () => {
    if (candlesLit === totalCandles && balloonsPoppedCount === totalBalloons && birthdayWish && !wished) {
      setWished(true)
    }
  }
          ...
          <CardContent className="space-y-6 text-center">
            ...
            <div>
              ...
              <input 
                ...
                className="border border-gray-400 rounded px-3 py-2 text-center w-full"
              />
              <Button 
                className="bg-black text-white hover:bg-gray-800 transition-all duration-300 mt-2"
                onClick={handleWish}
                disabled={wished}
              >
                Submit Wish
            ...
            {/* Display wish */}
            {wished && (
              <p className="text-lg font-semibold text-green-600 mt-4">&quot;{birthdayWish}&quot;</p>
            )}
          </CardContent>
